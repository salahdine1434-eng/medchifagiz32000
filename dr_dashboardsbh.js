...
/* ════════════════════════════════════════════════════════════
   ...
   ونفس منطق/أسلوب البحث المستخدم أعلاه لاختيار الطبيب الرئيسي،
   لإضافة طبيب واحد أو أكثر كمشاركين على شكل بطاقات (Chips).
...
════════════════════════════════════════════════════════════ */
(function () {
    let cncColabDebounce = null;
    let cncColabAbortCtrl = null;
    const cncColabSelected = new Map(); // doctorId (String) -> full_name

    function cncColabAvatarHtml(doc) {
        if (doc.avatar) {
            return '<div class="cnc-ext-doc-avatar"><img src="' + doc.avatar + '" alt=""></div>';
        }
        return '<div class="cnc-ext-doc-avatar"><i class="fas fa-user-doctor"></i></div>';
    ...
    function cncColabAddDoctor(doc) {
        const id = String(doc.id);
        const primaryId = cncColabGetPrimaryDoctorId();

        if (primaryId && id === primaryId) return; // يمنع إضافة الطبيب الرئيسي ضمن المشاركين
    ...
    function cncColabRenderResults(doctors) {
        const container = document.getElementById("cncColabDoctorResults");
        if (!container) return;
        ...
        doctors.forEach(function (doc) {
            ...
            const isSelected = cncColabSelected.has(id);

            const card = document.createElement("div");
            card.className = "cnc-ext-doc-card" +
                (isSelected ? " cnc-ext-doc-selected" : "") +
                (isPrimary ? " cnc-colab-doc-disabled" : "");
            card.setAttribute("data-doctor-id", id);

            let badge = "";
            if (isPrimary) {
                badge = '<span class="cnc-colab-doc-badge">الطبيب الرئيسي</span>';
            } else if (isSelected) {
                badge = '<span class="cnc-colab-doc-badge cnc-colab-doc-badge-added">مُضاف</span>';
            }

            card.innerHTML =
                cncColabAvatarHtml(doc) +
                '<div class="cnc-ext-doc-info">' +
                    '<div class="cnc-ext-doc-name">' + doc.full_name + '</div>' +
                    '<div class="cnc-ext-doc-meta">' + cncColabMetaHtml(doc) + '</div>' +
                '</div>' +
                badge;

            if (!isPrimary && !isSelected) {
                card.addEventListener("click", function () {
                    cncColabAddDoctor(doc);
                    cncColabRenderResults(doctors);
                });
            }
    ...
    async function cncColabRunSearch(query) {
        const container = document.getElementById("cncColabDoctorResults");
        if (!container) return;

        if (cncColabAbortCtrl) cncColabAbortCtrl.abort();
        cncColabAbortCtrl = new AbortController();

        try {
            const resp = await fetch("search_doctors.php?q=" + encodeURIComponent(query), {
                signal: cncColabAbortCtrl.signal
            });
            const data = await resp.json();
            if (!data.success) return;
            cncColabRenderResults(data.doctors || []);
        } catch (e) {
            if (e.name !== 'AbortError') console.error(e);
        }
    }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        input.addEventListener("input", function () {
            ...
            clearTimeout(cncColabDebounce);

            if (query === "") {
                container.innerHTML = '<div class="cnc-ext-doc-hint"><i class="fas fa-circle-info"></i> ابدأ بكتابة اسم الطبيب أو التخصص أو الولاية لإضافته كمشارك.</div>';
                return;
            }

            cncColabDebounce = setTimeout(function () {
                cncColabRunSearch(query);
            }, 300);
        });
    });
})();
