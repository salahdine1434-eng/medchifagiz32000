...
/* ===== التحقق من الصلاحية: أحد طرفي المحادثة فقط =====
   - الطبيب المُنشئ (created_by) — نوعه يُستَنتَج من clinic_id على الحالة
     نفسها (وُجدت ضمن عيادة ⇐ طبيب عيادة، وإلا فطبيب خاص)، وليس من نوع
     الجلسة الحالية، لأن الطرف الآخر (المختار) قد يفتح نفس الحالة بنوع حساب
     مختلف عن المُنشئ.
...
$creator_type = (intval($r['clinic_id']) > 0) ? 'clinic_staff' : 'private';
$is_creator   = ($my_type === $creator_type) && ($my_id === intval($r['created_by']));
