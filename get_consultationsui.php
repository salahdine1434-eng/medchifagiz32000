...
/* ===== تصنيف كل استشارة إلى مرسلة/واردة ===== */
$sent  = [];
$inbox = [];
...
foreach ($rows as $r) {
    ...
    $is_inbox = ($r['assigned_doctor_type'] === $my_type)
        && (intval($r['assigned_doctor_id']) === $my_id)
        && !$is_sent;
...
echo json_encode([
    ...
    'sent'          => $sent,
    'inbox'         => $inbox,
    'counts'        => ['sent' => cnsScopeCounts($sent), 'inbox' => cnsScopeCounts($inbox)],
