 ...
 * - يُسمح بالوصول: الطبيب المُنشئ، الطبيب المختار، أو أي طبيب مشارك
 *   (consultation_participants).
 * - لكل رسالة: اسم المرسل، وقتها، is_mine، ونوعها (type: text|image) مع
 *   رابط الملف (file_url) إن كانت من نوع image.
...
/* ===== ضمان وجود جدول الرسائل (+ عمودا type / file_path لدعم رسائل الصور) ===== */
try {
    $pdo->exec("
...
/* أعمدة رسائل الصور — تُضاف فقط إن لم تكن موجودة (جدول موجود من مرحلة سابقة) */
try {
...
/* ===== التحقق من صلاحية الوصول (منشئ، طبيب مختار، أو طبيب مشارك) ===== */
try {
    $stmt = $pdo->prepare("
...
try {
    $sql = "
        SELECT
            m.id, m.sender_id, m.sender_type, m.message, m.type, m.file_path, m.created_at,
...
$messages = [];
foreach ($rows as $r) {
    $type = (isset($r['type']) && $r['type'] === 'image') ? 'image' : 'text';
    $messages[] = [
        'id'          => intval($r['id']),
        ...
        'text'        => $r['message'],
        'type'        => $type,
        'file_url'    => ($type === 'image' && !empty($r['file_path'])) ? $r['file_path'] : null,
