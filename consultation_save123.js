...
(function () {
    ...
    function buildCard(c) {
        ...
        var docName     = (c.assigned_doctor_name && String(c.assigned_doctor_name).trim() !== "")
                            ? esc(c.assigned_doctor_name) : "—";
    ...
    function sentView()  { return document.querySelector('.cslt-view[data-status="sent"]'); }
    function sentList()  { var v = sentView(); return v ? v.querySelector(".consult-list") : null; }

    function currentScope() {
        // ملاحظة (السبب الجذري لعدم ظهور بطاقات "المرسلة"):
        // مدخلات التبديل الموجودة في dr_dashboard.php هي:
        //   <input type="radio" name="cslt-scope" id="cslt-scope-internal" ...>
        //   <input type="radio" name="cslt-scope" id="cslt-scope-external" ...>
        // بدون أي attribute باسم value. في هذه الحالة يكون r.value في
        // المتصفح دائماً القيمة الافتراضية "on" (سلوك HTML قياسي لعناصر
        // radio/checkbox بلا value)، وليس "internal"/"external" كما كان
        // مفترضاً. لذلك applyScopeFilter() كانت تقارن data-scope="internal"
        // أو "external" (القادمة من قاعدة البيانات عبر get_consultations.php)
        // بالقيمة "on" التي لا تطابق أياً منهما أبداً، فتُخفى كل البطاقات
        // ويظهر دائماً "لا توجد استشارات مرسلة" حتى لو كانت البيانات محفوظة
        // فعلياً في consultation_cases.
        // الحل: نعتمد على معرّف الـ radio نفسه بدل value، دون أي حاجة لتعديل
        // dr_dashboard.php (التصميم/الـ HTML يبقى كما هو تماماً).
        var r = document.querySelector('input[name="cslt-scope"]:checked');
        if (!r) return "internal";
        return (r.id === "cslt-scope-external") ? "external" : "internal";
    }

    // حالة فارغة داخل تبويب "المرسلة" (تعيد استخدام أصناف التصميم الموجودة)
    function ensureEmptyNode() {
        var v = sentView();
        if (!v) return null;
        var empty = v.querySelector(".cns-sent-empty");
        if (!empty) {
            empty = document.createElement("div");
            empty.className = "cslt-empty cns-sent-empty";
            empty.innerHTML =
                '<div class="cslt-empty-icon"><i class="fas fa-paper-plane"></i></div>' +
                '<h3>لا توجد استشارات مرسلة</h3>' +
                '<p>ابدأ بإنشاء استشارة جديدة لتظهر هنا كبطاقة حالة.</p>' +
    ...
    // إظهار/إخفاء البطاقات حسب النطاق النشط (داخلية/خارجية) + إدارة الحالة الفارغة
    function applyScopeFilter() {
        var list = sentList();
        if (!list) return;
        var scope = currentScope();
        var cards = list.querySelectorAll(".case-card-link");
        var visible = 0;
        ...
        if (empty) {
            ...
            // تحديث نص الحالة الفارغة حسب النطاق
            var h = empty.querySelector("h3");
            if (h) h.textContent = (scope === "external")
                ? "لا توجد استشارات خارجية مرسلة"
                : "لا توجد استشارات داخلية مرسلة";
        }
    }

    // رسم كل الاستشارات (المُرسَلة) داخل تبويب "المرسلة"
    function render(consultations) {
        var list = sentList();
        if (!list) return;
        var html = "";
        (consultations || []).forEach(function (c) { html += buildCard(c); });
        list.innerHTML = html;   // يستبدل أي بطاقات تجريبية سابقة
        applyScopeFilter();
    }

    // جلب الاستشارات من الخادم وعرضها
    function loadSent() {
        return fetch(LIST_URL, { credentials: "same-origin" })
            .then(function (r) { return r.json(); })
            .then(function (data) {
                if (data && data.success) {
                    render(data.consultations || []);
                }
                return data;
            })
            .catch(function (e) { console.error("loadSent failed", e); });
    }
    ...
    function handleCreate(btn) {
        ...
        .then(function (data) {
            if (data && data.success) {
                ...
                loadSent().then(applyScopeFilter);
            } else {
    ...
    function init() {
        ...
        document.querySelectorAll('input[name="cslt-scope"]').forEach(function (r) {
            r.addEventListener("change", applyScopeFilter);
        });
        // إعادة الفلترة عند العودة إلى تبويب "المرسلة"
        var sentTab = document.getElementById("cslt-status-sent");
        if (sentTab) sentTab.addEventListener("change", applyScopeFilter);
