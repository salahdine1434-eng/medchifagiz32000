 ...
 * أو التي هو فيها الطبيب المختار، أو التي هو فيها طبيب مشارك
 * (consultation_participants) — تحديث "إضافة طبيب مشارك".
...
/* ===== التحقق من الصلاحية: أحد أطراف المحادثة فقط =====
   - الطبيب المُنشئ (created_by).
   - الطبيب المختار (assigned_doctor_id / assigned_doctor_type).
   - أي طبيب مشارك (consultation_participants) — تحديث "إضافة طبيب مشارك". */
...
/* ===== قائمة المشاركين الفعليين لهذه الحالة (مع الأسماء) ===== */
$participants = [];
try {
    $ps = $pdo->prepare("
        SELECT
            cp.doctor_id, cp.doctor_type,
            COALESCE(cs.full_name, us.full_name) AS full_name
        FROM consultation_participants cp
        LEFT JOIN clinic_staff cs ON cs.id = cp.doctor_id AND cp.doctor_type = 'clinic_staff'
        LEFT JOIN users us        ON us.id = cp.doctor_id AND cp.doctor_type = 'private'
        WHERE cp.consultation_case_id = ?
        ORDER BY cp.created_at ASC
    ");
    $ps->execute([$id]);
    foreach ($ps->fetchAll(PDO::FETCH_ASSOC) as $prow) {
        $participants[] = [
            'id'        => intval($prow['doctor_id']),
            'type'      => $prow['doctor_type'],
            'full_name' => ($prow['full_name'] !== null && trim($prow['full_name']) !== '') ? $prow['full_name'] : 'طبيب',
        ];
    }
} catch (PDOException $e) {
    // الجدول قد لا يكون موجوداً بعد إن لم يُضَف أي مشارك سابقاً — غير حرج
}

$is_participant = false;
foreach ($participants as $p) {
    if ($p['id'] === $my_id && $p['type'] === $my_type) { $is_participant = true; break; }
}
...
/* نوع حساب المُنشئ (لأجل جلب اسمه ولمنع إضافته كمشارك من الواجهة): إن كان
   الفاتح الحالي هو المُنشئ نفسه فنوعه هو نوع جلسته الحالية بالضبط. وإلا
   يُحدَّد بالبحث الفعلي عن الحساب في الجدولين. */
if ($is_creator) {
...
/* عدّادات — المرفقات لا جدول لها بعد، المشاركون = الطبيب الرئيسي (إن وُجد) +
   عدد صفوف consultation_participants، أما الرسائل فمن consultation_messages */
$attachments_count = 0;
$participants_count = count($participants)
    + (($r['assigned_doctor_name'] !== null && trim($r['assigned_doctor_name']) !== '') ? 1 : 0);
...
echo json_encode([
    ...
    'consultation' => [
        ...
        'assigned_doctor_id'    => intval($r['assigned_doctor_id']),
        'assigned_doctor_type'  => $r['assigned_doctor_type'],
